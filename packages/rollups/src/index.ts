...
export const ROLLUPS: Record<number, Record<string, Rollup>> = {
  // Mainnet
  1: {
...
export function resolveRollup(address: string, chainId: number): Rollup | null {
  const normalizedAddress = address.toLowerCase();
  const addressMapping = ROLLUPS[chainId];

  if (!addressMapping) {
    return null;
  }

  return addressMapping[normalizedAddress] || null;
}
