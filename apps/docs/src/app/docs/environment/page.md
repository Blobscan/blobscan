...
| `NETWORK_NAME`                                         | Network's name (valid values are: `mainnet`, `hoodi`, `sepolia`, `gnosis`, `chiado`, `devnet`)     | No                                    | `mainnet`                                    |
...
| `GOOGLE_STORAGE_SIGNED_URLS_ENABLED`                   | Return signed URLs for Google Cloud Storage blobs in API responses                                 | No                                    | `false`                                      |
...
| `SWARMYCLOUD_STORAGE_ENABLED`                          | Store blobs in Swarmy service                                                                      | No                                    | `false`                                      |
| `SWARMY_API_KEY`                                       | API key required to authenticate requests to the Swarmy service                                    | If `SWARMYCLOUD_STORAGE_ENABLED=true` | (empty)                                      |
...
| `IPFS_STORAGE_ENABLED`                                 | Enable read-only IPFS blob retrieval via a gateway (CID references registered by blobscan-ipld)   | No                                    | `false`                                      |
| `IPFS_STORAGE_GATEWAY_URL`                               | IPFS gateway base URL used to fetch blobs                                                          | If `IPFS_STORAGE_ENABLED=true`        | `https://ipfs.filebase.io`                   |
| `IPFS_STORAGE_IPLD_URL`                               | blobscan-ipld service write API base URL (`POST /blob`), used to store blobs; distinct from the read gateway | If `PRIMARY_BLOB_STORAGE=ipfs`  | (empty)                                      |
| `IPFS_STORAGE_API_KEY`                                 | Bearer token sent to gated IPFS gateways (e.g. Filebase, Infura)                                    | No                                    | (empty)                                      |
| `IPFS_API_KEY`                                         | Bearer token authenticating inbound blobscan-ipld requests to the IPFS references endpoint          | No                                    | (empty)                                      |
...
| `NETWORK_NAME`            | Automatically start from the slot when blobs were activated (valid values are: `mainnet`, `hoodi`, `sepolia`, `gnosis`, `chiado`, `devnet`) | No       | `mainnet`           |
| `DENCUN_FORK_SLOT`        | Custom slot when blobs are activated (use when `NETWORK_NAME=devnet`)                                                                       | No       | (empty)             |
