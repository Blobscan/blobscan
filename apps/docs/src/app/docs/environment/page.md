...
| `IPFS_STORAGE_API_URL`                                | blobscan-ipld node write API base URL (`POST /blob`), used to store blobs; distinct from the read gateway | If `PRIMARY_BLOB_STORAGE=ipfs`  | (empty)                                      |
