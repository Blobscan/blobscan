...
import { useDimensions } from "~/hooks/useElementSizes";

type FadeableProps = {
  children: ReactNode;
};

const OPACITY_THRESHOLD = 0.25;
  ...
  return (
    <div className="relative">
      {isXOverflow && (
        <>
          <a.div
            className="absolute -left-0.5 z-10 w-3 bg-gradient-to-l from-transparent to-surface-light dark:to-surface-dark"
            style={{
              height: dimensions ? `${dimensions.height}px` : 0,
              opacity: scrollXProgress.to((v) => v / OPACITY_THRESHOLD),
            }}
          />
          <a.div
            className="absolute -right-0.5 z-10 w-3 bg-gradient-to-r from-transparent to-surface-light dark:to-surface-dark"
            style={{
              height: dimensions ? `${dimensions.height}px` : 0,
              opacity: scrollXProgress.to((v) => (1 - v) / OPACITY_THRESHOLD),
            }}
          />
        </>
      )}
      {isYOverflow && (
        <>
          <a.div
            className="absolute top-0 z-10 h-3 w-full bg-gradient-to-t from-transparent to-surface-light dark:to-surface-dark"
            style={{
              height: dimensions
