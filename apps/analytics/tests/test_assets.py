...
HOURLY_PARTITION_KEY = "2025-02-05-16:00"
HOURLY_PERIOD_START = datetime(2025, 2, 5, 16, 0, 0, tzinfo=timezone.utc)
HOURLY_PERIOD_END = HOURLY_PERIOD_START + timedelta(hours=1)

DAILY_PARTITION_KEY = "2025-02-01"
DAILY_PERIOD_START = datetime(2025, 2, 1, 0, 0, 0, tzinfo=timezone.utc)
...
WEEKLY_PERIOD_START = datetime(2025, 2, 3, 0, 0, 0, tzinfo=timezone.utc)
WEEKLY_PERIOD_END = WEEKLY_PERIOD_START + timedelta(days=7)

MONTHLY_PARTITION_KEY = "2025-01"
MONTHLY_PERIOD_START = datetime(2025, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
MONTHLY_PERIOD_END = datetime(2025, 2, 1, 0, 0, 0, tzinfo=timezone.utc)
    ...
    for tx in _FIXTURES["txs"]:
        ...
        result.append({
            **tx,
            ...
            "blob_as_calldata_max_fee": tx["blobAsCalldataGasUsed"] * tx["maxFeePerBlobGas"],
        })
...
def get_enriched_blobs(txs_in_window: list[dict]) -> list[dict]:
    """
    Return fixture blob-tx pairs for the given transactions, enriched with
    blob size fields and the block_number at the time of the tx (needed for
    the unique-blob calculation).
    """
...
def _run_asset(postgres: PostgresResource, partition_key: str, asset: dg.AssetsDefinition):
...
def clean_metrics(postgres):
    """Wipe all metrics tables before (and after) each test for full isolation."""
    with postgres.get_connection() as conn:
...
class _BlobMetricsMixin:
    ...
    def setup_metrics(self, postgres, clean_metrics):
    ...
    def test_total_unique_blobs(self, postgres):
        expected = sum(1 for b in self._blobs if b["firstBlockNumber"] == b["blockNumber"])
...
class _TxMetricsMixin:
    ...
    def test_global_only_metrics_are_null_for_non_global_rows(self, postgres):
        global_only_fields = ["avg_blob_gas_price", "total_blob_gas_price", "total_blocks"]
        for (cat, rollup), row in self._result(postgres).items():
            if (cat, rollup) != (None, None):
                for field in global_only_fields:
                    assert row[field] is None, (
                        f"{field} should be NULL for (category={cat!r}, rollup={rollup!r})"
                    )
    ...
    def test_total_unique_senders(self, postgres):
        expected = len({
            tx["fromId"] for tx in self._txs
            if _ADDR_MAP.get(tx["fromId"], {}).get("firstBlockNumberAsSender") == tx["blockNumber"]
        })
        assert self._result(postgres)[(None, None)]["total_unique_senders"] == expected

    def test_total_unique_receivers(self, postgres):
        expected = len({
            tx["toId"] for tx in self._txs
            if _ADDR_MAP.get(tx["toId"], {}).get("firstBlockNumberAsReceiver") == tx["blockNumber"]
        })
        assert self._result(postgres)[(None, None)]["total_unique_receivers"] == expected
    ...
    def test_total_blob_as_calldata_gas_used(self, postgres):
        ...
        assert self._result(postgres)[(None, None)]["total_blob_as_calldata_gas_used"] == expected
    ...
    def test_avg_blob_as_calldata_fee(self, postgres):
        ...
        assert self._result(postgres)[(None, None)]["avg_blob_as_calldata_fee"] == expected

    def test_total_blob_as_calldata_fee(self, postgres):
        expected = sum(tx["blob_as_calldata_fee"] for tx in self._txs)
        assert self._result(postgres)[(None, None)]["total_blob_as_calldata_fee"] == expected

    def test_avg_blob_as_calldata_max_fee(self, postgres):
        expected = sum(tx["blob_as_calldata_max_fee"] for tx in self._txs) / len(self._txs)
        assert self._result(postgres)[(None, None)]["avg_blob_as_calldata_max_fee"] == expected

    def test_total_blob_as_calldata_max_fee(self, postgres):
        expected = sum(tx["blob_as_calldata_max_fee"] for tx in self._txs)
        assert self._result(postgres)[(None, None)]["total_blob_as_calldata_max_fee"] == expected
