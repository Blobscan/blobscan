...
# Must be set before any module import that calls get_partition_start_date() at module level.
os.environ.setdefault("DAGSTER_METRICS_START_DATE", "2024-03-13-13:55")
...
def postgres():
    ...
    with PostgresContainer("postgres:16", username=DB_USERNAME, password=DB_PASSWORD, dbname=DB_NAME).with_bind_ports(5432, 9999) as container:
        url = (
        ...
        logger.debug(f"Postgres container started. Connection url {url}")
