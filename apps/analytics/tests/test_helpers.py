...
class TestGetPartitionStartDate:
    def test_valid_datetime_format_env_var(self, monkeypatch):
        monkeypatch.setenv("DAGSTER_METRICS_START_DATE", "2024-03-13-13:55")
        monkeypatch.delenv("CHAIN_ID", raising=False)
        result = get_partition_start_date()
        assert result == datetime.strptime("2024-03-13-13:55", "%Y-%m-%d-%H:%M")

    def test_valid_date_format_env_var(self, monkeypatch):
        monkeypatch.setenv("DAGSTER_METRICS_START_DATE", "2024-03-13")
        monkeypatch.delenv("CHAIN_ID", raising=False)
        result = get_partition_start_date()
        assert result == datetime.strptime("2024-03-13", "%Y-%m-%d")

    def test_invalid_env_var_raises_value_error(self, monkeypatch):
        monkeypatch.setenv("DAGSTER_METRICS_START_DATE", "not-a-date")
        with pytest.raises(ValueError, match="Invalid DAGSTER_METRICS_START_DATE format"):
            get_partition_start_date()

    @pytest.mark.parametrize(
        "chain_id,expected",
        [
            (1, datetime(2024, 3, 13, 13, 55, tzinfo=timezone.utc)),
            (11155111, datetime(2024, 1, 30, 22, 51, tzinfo=timezone.utc)),
            (560048, datetime(2025, 3, 17, 12, 0, tzinfo=timezone.utc)),
            (100, datetime(2024, 3, 11, 18, 30, tzinfo=timezone.utc)),
        ],
    )
    def test_known_chain_id_returns_dencun_activation_date(self, monkeypatch, chain_id, expected):
    ...
    def test_unknown_chain_id_raises_value_error(self, monkeypatch):
        monkeypatch.delenv("DAGSTER_METRICS_START_DATE", raising=False)
        monkeypatch.setenv("CHAIN_ID", "9999")
        with pytest.raises(ValueError, match="Partition start date could not be determined"):
            get_partition_start_date()
...
class TestExecuteSqlWindow:
    def _make_postgres_mock(self, rowcount: int):
    ...
    def test_returns_rowcount_and_elapsed_ms(self):
        ...
        mock_postgres, _ = self._make_postgres_mock(rowcount=42)

        rowcount, ms = execute_sql_window(context=context, postgres=mock_postgres, sql=text("SELECT 1"))
    ...
    def test_passes_partition_window_as_params(self):
        ...
        end = datetime(2024, 6, 1, 11, 0, tzinfo=timezone.utc)
        context = self._make_context(start, end)
        mock_postgres, mock_conn = self._make_postgres_mock(rowcount=0)
    ...
    def test_commits_after_execution(self):
        ...
        mock_postgres, mock_conn = self._make_postgres_mock(rowcount=5)

        execute_sql_window(context=context, postgres=mock_postgres, sql=text("SELECT 1"))
